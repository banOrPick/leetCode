...
public class 模拟目录管理 {
    ...
    static class DirectoryNode {
        ...
        // 创建子目录
        public DirectoryNode createChild(String name) {
            String childPath = this.path + name + "/";
            DirectoryNode child = new DirectoryNode(childPath, this);
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNextLine()) {
            ...
            switch (command) {
                case "mkdir":
                    if (parts.length == 2 && isValidDirectoryName(parts[1])) {
                        current.createChild(parts[1]);
                    }
                    break;
                case "cd":
                    if (parts.length == 2 && isValidChangeDirectory(parts[1])) {
                        if ("..".equals(parts[1])) {
                            if (current.parent != null) {
                                current = current.parent;
                            }
                        } else {
                            DirectoryNode child = current.getChild(parts[1]);
                            if (child != null) {
                                current = child;
                            }
                        }
    ...
    private static boolean isValidChangeDirectory(String name) {
        return "..".equals(name) || isValidDirectoryName(name);
    }
