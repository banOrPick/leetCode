    ...
    public static void main(String[] args) {
        ...
        for (char c : inputStr.toCharArray()) {
            if (c == '(') {
                ...
                if (!stack.isEmpty()) {
                    currentGroup.add(Character.toLowerCase(c));
                }
            }
        }
        ...
        for (List<Character> group : groups) {
            ...
            char first = group.get(0);
            int firstIdx = first - 'a';
            for (int i = 1; i < group.size(); i++) {
                char c = group.get(i);
                int cIdx = c - 'a';
                uf.union(firstIdx, cIdx);
            }
        }
        ...
        for (char c : inputStr.toCharArray()) {
            if (c == '(') {
                ...
                if (!inBracket) {
                    ...
                    char minChar = (char) ('a' + root);
                    result.append(minChar);
                }
            }
        }

        String output = result.length() > 0 ? result.toString() : "0";
